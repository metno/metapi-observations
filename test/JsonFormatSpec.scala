...
class JsonFormatSpec extends Specification {
  ...
  def createJsonLd(): JsValue = {
    ...
    val data = ObservationSeries(
      Some(station), None, Some("2015-02-01T06:00:00Z"), Some(List(new Observation(Some("air_temperature"),
        Some(ObsValue(12.7)), Some("degC"), None, None, Some("1"), Some("A"), Some(0), None))))
