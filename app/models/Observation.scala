...
case class Observation(
  ...
  @(ApiModelProperty @field)(value = "A link to the complete quality description for this component.", example = "/quality/v0.jsonld?flags=70000") quality: Option[URL],
        ...
        if (!value.isEmpty) {
          l :+ new ObservationSeries(
            ...
            Some(Seq(Observation(Some(meta.get.elementId),
              ...
              quality match {
                case Some(q) => {
                  val qualityLookup = controllers.routes.ObservationsController.quality(q, None, None, "jsonld");
                  Some(new URL(ConfigUtil.urlStart + qualityLookup.url.substring(1)))
                }
                case None => None
              },
              None))))
